...
function DocumentPreview({ doc }) {
  ...
  if (extension === "pdf") {
    ...
    return <pre className="whitespace-pre-wrap text-sm text-gray-800 dark:text-gray-100">{decoded}</pre>;
  ...
  return (
    <div className="flex h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <main className="flex-1 p-8 overflow-auto">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">{folder.name}</h1>
        <p className="text-gray-600 dark:text-gray-300 mb-4 italic">{folder.description}</p>
        ...
        <div className="mb-8">
          <div className="bg-gradient-to-br from-blue-100 to-blue-50 dark:from-gray-800 dark:to-gray-700 border border-blue-200 dark:border-gray-600 shadow-sm rounded-2xl p-6">
            <h2 className="text-lg font-bold text-blue-800 dark:text-white mb-4">Informations sur le dossier</h2>
            <div className="grid sm:grid-cols-2 gap-4 text-sm text-gray-800 dark:text-gray-200">
              <div><span className="font-semibold">Client :</span> {folder.client_name || "—"}</div>
              <div><span className="font-semibold">Statut :</span> {folder.status}</div>
              <div><span className="font-semibold">Date de soumission :</span> {folder.submission_deadline?.split("T")[0] || "—"}</div>
              <div><span className="font-semibold">Créé le :</span> {new Date(folder.created_at).toLocaleDateString()}</div>
              <div><span className="font-semibold">Nombre de documents :</span> {folder.document_count}</div>
            </div>
          </div>
        </div>

        <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-4 border-b pb-2">Documents</h2>
        ...
        {selectedDoc && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 px-4">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl max-w-4xl w-full h-[90vh] relative overflow-auto">
              ...
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">{selectedDoc.filename}</h3>
