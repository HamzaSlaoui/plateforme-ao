...
const ChatbotPage = () => {
  ...
  const [messages, setMessages] = useState([
    {
      id: '1',
      message: 'Bonjour ! Je suis votre assistant IA pour ce dossier. Je peux répondre à vos questions sur les documents, les exigences, et vous aider dans votre préparation. Comment puis-je vous aider ?',
      isUser: false,
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage = {
        id: (Date.now() + 1).toString(),
        message: "Désolé, une erreur s'est produite lors du traitement de votre message. Veuillez réessayer.",
        isUser: false,
        timestamp: new Date().toISOString(),
      };
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 p-4">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              {folderInfo && (
                <span>
                  {folderInfo.document_count} document{folderInfo.document_count !== 1 ? 's' : ''}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              onClick={() => setInputMessage("Quels sont les critères d'évaluation de cet appel d'offres ?")}
            ...
            <button
              onClick={() => setInputMessage("Quelle est la date limite de soumission ?")}
              className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
              disabled={isLoading}
            >
              Date limite
            ...
            <button
              onClick={() => setInputMessage("Quels documents dois-je fournir ?")}
              className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
              disabled={isLoading}
            >
              Documents requis
            ...
            <button
              onClick={() => setInputMessage("Résume-moi les points clés de ce dossier")}
              className="text-xs bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-3 py-1 rounded-full hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors"
              disabled={isLoading}
            >
              Points clés
